...
def main(seed=42, steps=4000):
    ...
    # Optimization loop
    def run_quadratic(optimizer_cls, method='qdwh', steps=steps, lr=0.05, scheduler=False, momentum=False, polar_first=False):
        torch.manual_seed(seed)
    ...
    loss_polar_grad_polar_first, cond_polar_grad_polar_first, cond_grad_polar_grad_polar_first, nuc_polar_grad_polar_first = run_quadratic(PolarGrad, method='qdwh', lr=4e-7, momentum=True, polar_first=True)
    loss_polar_grad_polar_first_decay, cond_polar_grad_polar_first_decay, cond_grad_polar_grad_polar_first_decay, nuc_polar_grad_polar_first_decay = run_quadratic(PolarGrad, method='qdwh', lr=5e-7, momentum=True, polar_first=True, scheduler=True)
    loss_polar_grad_momentum_first, cond_polar_grad_momentum_first, cond_grad_polar_grad_momentum_first, nuc_polar_grad_momentum_first = run_quadratic(PolarGrad, method='qdwh', lr=2e-7, momentum=True)
    loss_polar_grad_momentum_first_decay, cond_polar_grad_momentum_first_decay, cond_grad_polar_grad_momentum_first_decay, nuc_polar_grad_momentum_first_decay = run_quadratic(PolarGrad, method='qdwh', lr=2.5e-7, momentum=True, scheduler=True)
    ...
    axes[0].semilogy(loss_polar_grad_quad, label="PolarGrad", linestyle='--')
    axes[0].semilogy(loss_polar_grad_quad_decay, label=r"PolarGrad (lr $\downarrow$)", linestyle=':')
    axes[0].semilogy(loss_polar_grad_polar_first, label="PolarGradM (polar-first)", linestyle='-.')
    axes[0].semilogy(loss_polar_grad_polar_first_decay, label=r"PolarGradM (polar-first; lr $\downarrow$)", linestyle=':')
    axes[0].semilogy(loss_polar_grad_momentum_first, label="PolarGradM (momentum-first)", linestyle='-.')
    axes[0].semilogy(loss_polar_grad_momentum_first_decay, label=r"PolarGradM (momentum-first; lr $\downarrow$)", linestyle=':')
    axes[0].semilogy(loss_muon_quad_qdwh, label="Muon", linestyle='--')
    axes[0].semilogy(loss_muon_quad_qdwh_decay, label=r"Muon (lr $\downarrow$)", linestyle=':')
    ...
    axes[1].semilogy(cond_polar_grad_polar_first, linestyle='-.')
    axes[1].semilogy(cond_polar_grad_polar_first_decay, linestyle=':')
    axes[1].semilogy(cond_polar_grad_momentum_first, linestyle='-.')
    axes[1].semilogy(cond_polar_grad_momentum_first_decay, linestyle=':')
    ...
    axes[2].semilogy(cond_grad_polar_grad_polar_first, linestyle='-.')
    axes[2].semilogy(cond_grad_polar_grad_polar_first_decay, linestyle=':')
    axes[2].semilogy(cond_grad_polar_grad_momentum_first, linestyle='-.')
    axes[2].semilogy(cond_grad_polar_grad_momentum_first_decay, linestyle=':')
    ...
    fig.savefig(f'fig/mat_quad_reg_mom_{seed}.pdf', dpi=500, bbox_inches='tight')
    plt.close(fig)


    # Plot nuclear norms
    fig2 = plt.figure(figsize=(7, 5))
    plt.semilogy(nuc_polar_grad_quad, label="PolarGrad", linestyle='--')
    plt.semilogy(nuc_polar_grad_quad_decay, label=r"PolarGrad (lr $\downarrow$)", linestyle=':')
    plt.semilogy(nuc_polar_grad_polar_first, label="PolarGradM (polar-first)", linestyle='-.')
    plt.semilogy(nuc_polar_grad_polar_first_decay, label=r"PolarGradM (polar-first; lr $\downarrow$)", linestyle=':')
    plt.semilogy(nuc_polar_grad_momentum_first, label="PolarGradM (momentum-first)", linestyle='-.')
    plt.semilogy(nuc_polar_grad_momentum_first_decay, label=r"PolarGradM (momentum-first; lr $\downarrow$)", linestyle=':')
    plt.semilogy(nuc_muon_quad_qdwh, label="Muon", linestyle='--')
    plt.semilogy(nuc_muon_quad_qdwh_decay, label=r"Muon (lr $\downarrow$)", linestyle=':')
    ...
    fig2.legend(loc="outside lower center", ncol=4, bbox_to_anchor=(0.5, -0.05), borderaxespad=0., fontsize=16)
    fig2.subplots_adjust(bottom=0.22)
    fig2.savefig(f'fig/mat_quad_reg_mom_nuc_{seed}.pdf', dpi=500, bbox_inches='tight')
    plt.close(fig2)
