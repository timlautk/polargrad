...
class LowRankModelAltGD(nn.Module):
    ...
    def fit(self, target, mask, steps=200, num_inner_steps=1):
        """
...
def main(seed=42, steps=200):
    ...
    # Training loop
    def train_lowrank(optimizer_cls, method='qdwh', steps=steps, lr=0.1, scheduler=False, momentum=False, polar_first=False):
        torch.manual_seed(seed)
        model = LowRankModel()
        ...
        if optimizer_cls == torch.optim.Adam:
            ...
            if not momentum:
                ...
                if polar_first:
                    optimizer = optimizer_cls(model.parameters(), method=method, lr=lr, momentum=0.5, polar_first=True)
                else:
                    optimizer = optimizer_cls(model.parameters(), method=method, lr=lr, momentum=0.5)
        else:
    ...
    axes[0].semilogy(loss_polar_grad_lr, label="PolarGrad", linestyle='-')
    axes[0].semilogy(loss_polar_grad_lr_decay, label=r"PolarGrad (lr $\downarrow$)", linestyle='--')
    axes[0].semilogy(loss_polar_grad_lr_polar_first, label="PolarGradM (polar-first)", linestyle='-.')
    axes[0].semilogy(loss_polar_grad_lr_polar_first_decay, label=r"PolarGradM (polar-first; lr $\downarrow$)", linestyle='--')
    axes[0].semilogy(loss_polar_grad_lr_momentum_first, label="PolarGradM (momentum-first)", linestyle='-.')
    axes[0].semilogy(loss_polar_grad_lr_momentum_first_decay, label=r"PolarGradM (momentum-first; lr $\downarrow$)", linestyle='--')
    ...
    axes[1].plot(cond_X_polar_grad_lr_polar_first, linestyle='-.')
    axes[1].plot(cond_X_polar_grad_lr_polar_first_decay, linestyle='--')
    axes[1].plot(cond_X_polar_grad_lr_momentum_first, linestyle='-.')
    axes[1].plot(cond_X_polar_grad_lr_momentum_first_decay, linestyle='--')
    ...
    axes[2].plot(cond_Y_polar_grad_lr_polar_first, linestyle='-.')
    axes[2].plot(cond_Y_polar_grad_lr_polar_first_decay, linestyle='--')
    axes[2].plot(cond_Y_polar_grad_lr_momentum_first, linestyle='-.')
    axes[2].plot(cond_Y_polar_grad_lr_momentum_first_decay, linestyle='--')
    axes[2].set_xlabel(r"iteration $k$")
    axes[2].set_ylabel(r"$\kappa_2(\nabla_Y \mathsf{f}(X_k, Y_k))$")

    fig.legend(loc='outside lower center', ncol=6, bbox_to_anchor=(0.5, -0.05), borderaxespad=0., fontsize=16)
    fig.subplots_adjust(bottom=0.15)
    fig.savefig(f'fig/low_rank_mat_comp_mom_{seed}.pdf', dpi=500, bbox_inches='tight')
    plt.close(fig)


    fig2, axes2 = plt.subplots(1, 2, figsize=(14, 5))
    # Plot nuclear norms of gradients of X
    axes2[0].plot(nuc_X_polar_grad_lr[0:200], label="PolarGrad", linestyle='-')
    axes2[0].plot(nuc_X_polar_grad_lr_decay[0:200], label=r"PolarGrad (lr $\downarrow$)", linestyle='--')
    axes2[0].plot(nuc_X_polar_grad_lr_polar_first[0:200], label="PolarGradM (polar-first)", linestyle='-.')
    axes2[0].plot(nuc_X_polar_grad_lr_polar_first_decay[0:200], label=r"PolarGradM (polar-first; lr $\downarrow$)", linestyle='--')
    axes2[0].plot(nuc_X_polar_grad_lr_momentum_first[0:200], label="PolarGradM (momentum-first)", linestyle='-.')
    axes2[0].plot(nuc_X_polar_grad_lr_momentum_first_decay[0:200], label=r"PolarGradM (momentum-first; lr $\downarrow$)", linestyle='--')
    ...
    axes2[1].plot(nuc_Y_polar_grad_lr_polar_first[0:200], linestyle='-.')
    axes2[1].plot(nuc_Y_polar_grad_lr_polar_first_decay[0:200], linestyle='--')
    axes2[1].plot(nuc_Y_polar_grad_lr_momentum_first[0:200], linestyle='-.')
    axes2[1].plot(nuc_Y_polar_grad_lr_momentum_first_decay[0:200], linestyle='--')
    ...
    fig2.legend(loc='outside lower center', ncol=6, bbox_to_anchor=(0.51, -0.05), borderaxespad=0., fontsize=16)
    fig2.subplots_adjust(bottom=0.15)
    fig2.savefig(f'fig/low_rank_mat_comp_mom_3_{seed}.pdf', dpi=500, bbox_inches='tight')
    plt.close(fig2)
