...
class Muon_polar(torch.optim.Optimizer):
    def __init__(self, params, lr=0.02, momentum=0.95, nesterov=True, method='ns', inner_steps=5, a=3.4445, b=-4.7750, c=2.0315):
        defaults = dict(lr=lr, momentum=momentum, nesterov=nesterov, method=method, inner_steps=inner_steps, a=a, b=b, c=c)
        super().__init__(params, defaults)
    ...
    def step(self):
        for group in self.param_groups:
            ...
            a, b, c = group['a'], group['b'], group['c']
            for param in group['params']:
                g = param.grad
                # lr *= max(1, g.size(0) / g.size(1)) ** 0.5
                assert g is not None
                ...
                g = polar(g, method=method, max_iterations=inner_steps, ns_coeffs=(a, b, c))[0]
